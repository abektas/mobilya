    ...
    <header
      className={cn(
        ...
        scrolled
          ...
          : 'bg-gradient-to-b from-black/80 via-black/50 to-black/20'
      )}
    >
      {/* Top Bar */}
      <div className={cn(
        'hidden lg:flex items-center justify-between px-4 py-1.5 text-xs border-b transition-colors',
        scrolled ? 'border-gray-200/50 dark:border-gray-700/50 text-muted-foreground' : 'border-white/10 text-white/70'
      )}>
        <div className="max-w-[1400px] mx-auto w-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/tedarikciler" className="hover:text-primary transition-colors">{t.nav.becomeSupplier}</Link>
            <span className="text-muted-foreground/30">|</span>
            <Link href="/giris" className="hover:text-primary transition-colors">{t.nav.signIn}</Link>
            <Link href="/kayit" className="hover:text-primary transition-colors">{t.nav.signUp}</Link>
          </div>
        </div>
      </div>
      {/* Main Header */}
        ...
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="hidden lg:flex flex-1 max-w-2xl mx-8">
            <div className="relative w-full">
              <Input type="text" placeholder={t.hero.searchPlaceholder}
                ...
                className={cn(
                  'w-full h-11 pl-4 pr-12 rounded-xl border-2 transition-all',
                  scrolled
                    ? 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700 focus:border-primary'
                    : 'bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-white/60 focus:border-white/40'
                )}
              />
              <button className={cn(
                'absolute right-1.5 top-1/2 -translate-y-1/2 p-2 rounded-lg',
                scrolled ? 'hover:bg-gray-100 text-muted-foreground' : 'hover:bg-white/10 text-white'
              )}>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
                  ...
                  <motion.div initial={{ opacity: 0, y: 8 }}
                    ...
                    {categories.map((cat) => (
                      <Link key={cat.key} href={cat.href}
                        className="block px-3 py-2 rounded-lg text-sm hover:bg-primary/10 hover:text-primary">
            ...
            <Button variant="ghost" size="sm" onClick={toggleLang}
              className={cn('gap-1.5', !scrolled && 'text-white hover:bg-white/15')}>
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }}
            ...
            <nav className="max-w-[1400px] mx-auto px-4 py-4 flex flex-col gap-1">
              ...
              <Link href="/" onClick={() => setMobileOpen(false)}
                className="px-4 py-3 text-sm font-medium rounded-lg hover:bg-primary/10 hover:text-primary">
                {t.nav.home}
              </Link>
              <Link href="/urunler" onClick={() => setMobileOpen(false)}
                className="px-4 py-3 text-sm font-medium rounded-lg hover:bg-primary/10 hover:text-primary">
                {t.nav.products}
              </Link>
              <Link href="/tedarikciler" onClick={() => setMobileOpen(false)}
                className="px-4 py-3 text-sm font-medium rounded-lg hover:bg-primary/10 hover:text-primary">
                {t.nav.suppliers}
              </Link>
              <Link href="/teklif-iste" onClick={() => setMobileOpen(false)}
                className="px-4 py-3 text-sm font-medium rounded-lg hover:bg-primary/10 hover:text-primary">
                {t.nav.rfq}
              </Link>
              ...
              <Link href="/kayit" onClick={() => setMobileOpen(false)}>
                <Button className="w-full mt-2 bg-gradient-to-r from-amber-500 to-orange-500 text-white border-0">
                  {t.nav.signUp}
                </Button>
              </Link>
            </nav>
